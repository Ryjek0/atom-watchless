...
export default class Watchless{
  ...
  constructor(state) {    
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'atom-watchless:startGulpWatch': () => this.startGulp(true),
      'atom-watchless:generateAllLess': () => this.generateAllLess(),
      'atom-watchless:updateConfig': () => this.config.updateConfig()
    }));
    // console.log("start");
    this.config = new Config(atom.project.getPaths());
    if (atom.config.get(`atom-watchless.startWachOnStartup`)) {
      this.startGulp(true);
    }
  }
  ...
  startGulp(watch = false){
    ...
    const lessPaths = atom.config.get("atom-watchless.lessPaths"),
          stylesPaths = atom.config.get("atom-watchless.compiledPaths"),
    ...
    for(let less in arr){
      ...
      if (watch) {
        gulp.watch(`${rootDir}/${less}/*.less`).on("change",file => {
          const filePath = file.path,dirs = filePath.split("/"),
                name = dirs.pop();
          ...
          if (allLess) {
            ...
            thc.lessOneFile(filePath,cssFile,name);
          } else {
            thc.lessOneFile(filePath,`${rootDir}/${style}`,name);
          }
        });
      } else {
        let lessDir = less;
  ...
  lessOneFile (src,destDir,name) {
  	...
  	console.log("/////////////////////////////////");
    const showNotifOnSuccess = atom.config.get("atom-watchless.showNotifOnSuccess"),
          generateSourceMap = atom.config.get("atom-watchless.generateSourceMap"),
          minimaliseCss = atom.config.get("atom-watchless.minimaliseCss");
